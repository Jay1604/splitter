...
public class GruppenRepositoryImpl implements GruppenRepository {
  ...
  public GruppenRepositoryImpl(SpringDataPersonenRepository personenRepository,
  ...
  public de.hhu.propra.splitter.domain.models.Gruppe toGruppe(Gruppe gruppeDto) {
    Iterator<Mitglied> listMitglieder = gruppeDto.getMitglieder().iterator();
    de.hhu.propra.splitter.domain.models.Gruppe gruppe =
        new de.hhu.propra.splitter.domain.models.Gruppe(
            gruppeDto.getId().longValue(), getMitgliedName(listMitglieder.next()),
            gruppeDto.getName());
    ...
    for (Ausgabe ausgabe : gruppeDto.getAusgaben()) {
      gruppe.addAusgabe(ausgabe.beschreibung(), Money.of(ausgabe.betrag(), "EUR").divide(100),
          getPersonName(
              ausgabe.glaeubiger()),
          ausgabe.schuldner().stream().map(a -> getPersonName(a.person())).collect(
              Collectors.toSet()));
    }
    return gruppe;
  }
  ...
  private String getMitgliedName(Mitglied mitglied) {
    return personenRepository.findById(mitglied.person()).orElseThrow(PersonNotFoundException::new)
        .gitHubName();
  }

  private String getPersonName(int person) {
    return personenRepository.findById(person).orElseThrow(PersonNotFoundException::new)
        .gitHubName();
  }

  public Gruppe fromGruppe(de.hhu.propra.splitter.domain.models.Gruppe gruppe) {

    return new Gruppe(gruppe.getId().intValue(), gruppe.getName(), gruppe.isOffen(),
        gruppe.getMitglieder().stream().map(
                a -> new Mitglied(getOrCreatePersonId(a.getGitHubName()),
                    gruppe.getId().intValue()))
            .collect(Collectors.toSet()), gruppe.getAusgaben().stream()
        .map(a -> new Ausgabe(null, a.getBeschreibung(), a.getBetrag()
            .multiply(100)
            .getNumber().longValue(),
            getOrCreatePersonId(a.getGlaeubiger().getGitHubName()), a.getSchuldner().stream()
            .map(b -> new Schuldner(getOrCreatePersonId(b.getGitHubName()))).collect(
  ...
  private Integer getOrCreatePersonId(String name) {
    ...
    return personenRepository.save(new Person(null, name)).id();
  }
